...
def handle_topology_parameters():
    ...
    graph_model = st.selectbox("Graph Model", ['Erdos-Renyi', 'Barabasi-Albert', 'Watts-Strogatz', 'Mean Field'])
...
def __handle_epidemics_model_parameters():
    ...
    if model_type == 'SIS':
        config.add_model_parameter('lambda',
                                   st.slider("lambda (Recovery rate)", min_value=0.0, max_value=1.0, value=0.1))
    elif model_type == 'SIR':
...
def __handle_opinion_model_parameters():
    model_type = st.selectbox("Model", [#'Voter', 'QVoter', 'Sznajd',
                                        'Deffuant', 'Hegselmann-Krause', 'Algorithmic Bias'])
    model_method = MODELS[model_type]
    config = mc.Configuration()
    config.add_model_parameter('epsilon',
                               st.slider("epsilon (Bounded confidence threshold)", min_value=0.0, max_value=1.0,
                                         value=0.1))
    if model_type == 'Algorithmic Bias':
        config.add_model_parameter('gamma',
